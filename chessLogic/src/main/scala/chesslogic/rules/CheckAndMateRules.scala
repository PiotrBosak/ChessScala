...
object CheckAndMateRules {
  ...
  private def isKingAttacked(board:Board, kingColor:Color):Boolean = {
    val kingTile = board.tiles.values.find(t => t.currentPiece match {
    ...
    val possibleAttacks = board.tiles.values.collect(tile => {
  ...
  def cannotBeDefended(kingColor: Color,game:FullGame): Boolean = {
    ...
    val allPossibleScenarios = for {
      ...
      moveTo <- moveTuple._2
      newGame <- game.makeMoveWithoutTurn(moveFrom,moveTo)
    } yield newGame
