...
import lib.logic.rules.{BishopRules, CheckAndMateRules, KingRules, KnightRules, PawnRules, QueenRules, RookRules}
...
import MoveType.*
import lib.logic.pieces.Piece
import lib.logic.pieces.Piece.King
import io.circe.Codec

import scala.util.Try
import lib.logic.board.util.codec.{ decodeMap, encodeMap }

case class Board private (tiles: Map[Position, Tile], previousMove: Option[Move]) derives Codec.AsObject {
  ...
  private def isCastling(tileFrom: Tile, tileTo: Tile): Boolean = {
    tileFrom.currentPiece.exists {
      case _: King  => Math.abs(tileFrom.position.file.toNumber - tileTo.position.file.toNumber) >= 2
      case _: Piece => false
    }
  }
