...
class PawnRulesTest extends AnyFlatSpec {
  ...
  "Pawn on A2" can "make le passant" in {
    val first  = game.makeMoveWithoutTurn(Position(D, Two), Position(D, Four)).get
    val second = first.makeMoveWithoutTurn(Position(D, Four), Position(D, Five)).get
    val third  = second.makeMoveWithoutTurn(Position(E, Seven), Position(E, Five)).get
    assertResult(1)(PawnRules.getPossibleAttacks(Position(D, Five), third.currentBoard).size)
    val afterLePassant = third.makeMoveWithoutTurn(Position(D, Five), Position(E, Six)).get
  ...
  "Pawn on A2" should "not be able to do le passant" in {
    val first  = game.makeMoveWithoutTurn(Position(D, Two), Position(D, Four)).get
    val second = first.makeMoveWithoutTurn(Position(D, Four), Position(D, Five)).get
    val third  = second.makeMoveWithoutTurn(Position(E, Seven), Position(E, Six)).get
    val fourth = third.makeMoveWithoutTurn(Position(E, Six), Position(E, Five)).get
    assertResult(0)(PawnRules.getPossibleAttacks(Position(D, Five), fourth.currentBoard).size)
  }
