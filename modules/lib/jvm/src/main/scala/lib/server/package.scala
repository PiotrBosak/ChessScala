...
trait OrphanInstances {
  ...
  implicit val nonNegativeDecoder: Decoder[Int Refined NonNegative] =
    Decoder[Int].emap(i =>
      if (i >= 0) Right(Refined.unsafeApply[Int, NonNegative](i)) else Left("Passed number is negative")
    )
  ...
  implicit val nonEmptyStringDecoder: Decoder[String Refined NonEmpty] =
    Decoder[String].emap(s =>
      if (s.isEmpty) Left("name cannot be empty") else Right(Refined.unsafeApply[String, NonEmpty](s))
    )
