...
import lib.logic.rules.RulesForMultiTileMoves.{getViablePositionForAttacks, getViablePositionsForMoves}
...
object RookRules extends MovingRules[Rook] {
  ...
  override def getPossibleMoves(position: Position, board: Board): List[(MoveType, Position)] = {
    (getDownMoves(position, board) ++ getTopMoves(position, board) ++
      getLeftMoves(position, board) ++ getRightMoves(position, board)).map(p => (MoveType.Normal, p))
  }
