...
case object BoardFactory {
  ...
  private def createTilesWithBlackBishops(): Map[Position, Tile] = Map(
    Position(8, 3) -> Tile(Black, Position(8, 3), Some(Bishop(Black))),
    Position(8, 6) -> Tile(White, Position(8, 6), Some(Bishop(Black)))
  ).view.mapValues(_.get).toMap


  private def createTilesWithBlackKnights(): Map[Position, Tile] = Map(
    Position(8, 2) -> Tile(White, Position(8, 2), Some(Knight(Black))),
    Position(8, 7) -> Tile(Black, Position(8, 7), Some(Knight(Black)))
  ).view.mapValues(_.get).toMap

  private def createTilesWithBlackRooks(): Map[Position, Tile] = Map(
    Position(8, 1) -> Tile(Black, Position(8, 1), Some(Rook(Black))),
    Position(8, 8) -> Tile(White, Position(8, 8), Some(Rook(Black)))
  ).view.mapValues(_.get).toMap

  private def createTilesWithWhiteBishops(): Map[Position, Tile] = Map(
    Position(1, 3) -> Tile(White, Position(1, 3), Some(Bishop(White))),
    Position(1, 6) -> Tile(Black, Position(1, 6), Some(Bishop(White)))
  ).view.mapValues(_.get).toMap

  private def createTilesWithWhiteKnights(): Map[Position, Tile] = Map(
    Position(1, 2) -> Tile(Black, Position(1, 2), Some(Knight(White))),
    Position(1, 7) -> Tile(White, Position(1, 7), Some(Knight(White)))
  ).view.mapValues(_.get).toMap

  private def createTilesWithWhiteRooks(): Map[Position, Tile] = Map(
    Position(1, 1) -> Tile(White, Position(1, 1), Some(Rook(White))),
    Position(1, 8) -> Tile(Black, Position(1, 8), Some(Rook(White)))
  ).view.mapValues(_.get).toMap

  private def createTileWithBlackKing(): (Position, Tile) =
    Position(8, 5) -> Tile(Black, Position(8, 5), Some(King(Black))).get

  private def createTileWithBlackQueen(): (Position, Tile) =
    Position(8, 4) -> Tile(White, Position(8, 4), Some(Queen(Black))).get

  private def createTileWithWhiteKing(): (Position, Tile) =
    Position(1, 5) -> Tile(White, Position(1, 5), Some(King(White))).get

  private def createTileWithWhiteQueen(): (Position, Tile) =
    Position(1, 4) -> Tile(Black, Position(1, 4), Some(Queen(White))).get
  ...
  private def createTiles(row: Int, piece: Option[Piece]) = {
    ...
    def createTilesInColumn(column: Int, map: Map[Position, Tile]): Map[Position, Tile] = {
      if (column > 8) map
        ...
        println(Tile(White, Position(row, column), piece))
